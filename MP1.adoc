...
===== Without Memory Transfer Accounted
image::compute_time.png[]
===== With Memory Transfer Accounted
image::compute_time_transfer_accounted.png[]

As can be seen in the plots, calculating the matrix multiplication on the GPU is always faster than on the CPU. This is expected as the GPU is designed to perform parallel computations, and matrix multiplication is a highly parallelizable operation. A time when it is not beneficial to run the matrix multiplication on the GPU is when the matrix size is small, and the overhead of transferring data to and from the GPU is greater than the performance gains from calculating it on the GPU.
